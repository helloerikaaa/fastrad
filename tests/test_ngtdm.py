...
def to_fastrad_key(pyrad_key):
    if not pyrad_key.startswith("original_ngtdm_"):
        return None
    
    f_name = pyrad_key.replace("original_ngtdm_", "")
    # Convert camel case to snake case
    snake = re.sub(r'(?<!^)(?=[A-Z])', '_', f_name).lower()
    return f"ngtdm:{snake}"
...
def test_ngtdm_cpu():
    ...
    settings = FeatureSettings(feature_classes=["ngtdm"], bin_width=25.0, device="cpu")
    ...
    print("All NGTDM features match pyradiomics!")
