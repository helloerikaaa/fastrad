...
def to_fastrad_key(pyrad_key):
    ...
    f_name = pyrad_key.replace("original_glrlm_", "")
    # Convert camel case to snake case
    snake = re.sub(r'(?<!^)(?=[A-Z])', '_', f_name).lower()
    return f"glrlm:{snake}"
...
def test_glrlm_cpu():
    ...
    settings = FeatureSettings(feature_classes=["glrlm"], bin_width=25.0, device="cpu")
    ...
    print("All GLRLM features match pyradiomics!")
