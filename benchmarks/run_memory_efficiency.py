...
def run():
    ...
    def run_pyrad(i_sitk, m_sitk):
        pyrad_ext.execute(i_sitk, m_sitk)
        
    def run_fastrad(f_img, f_mask):
        f_ext = FeatureExtractor(fastrad_settings)
        f_ext.extract(f_img, f_mask)
    ...
    for r in [5, 10, 15, 20, 25, 30]:
        ...
        pyrad_mem = measure_peak_ram(run_pyrad, img_sitk, m_sitk)
        fastrad_mem = measure_peak_ram(run_fastrad, fastrad_img, f_mask)
